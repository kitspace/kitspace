...
if (require.main !== module) {
    module.exports = function(config) {
        let deps;
        const targets = ['build/privacy/index.html'];
        if (config === 'production') {
            deps = ['build/.temp/privacy/privacy.jsx', 'src/privacy/privacy.html'];
            return {deps, targets, moduleDep: true};
        }
        else if (config === 'dev') {
            deps = ['src/privacy/privacy.html'];
    ...
    if (config === 'production') {
        ...
        //just copy the privacy.html to index.html
        html = deps[0];
