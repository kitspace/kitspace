...
if (require.main !== module) {
    module.exports = function(config) {
        let deps;
        const targets = ['build/login/index.html'];
        if (config === 'production') {
            deps = ['build/.temp/login/login.jsx', 'src/login/login.html'];
            return {deps, targets, moduleDep: true};
        }
        else if (config === 'dev') {
            deps = ['src/login/login.html'];
    ...
    if (config === 'production') {
        ...
        //just copy the login.html to index.html
        html = deps[0];
